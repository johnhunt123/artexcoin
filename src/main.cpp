...
uint256 hashGenesisBlock("0x00000b9fea0dc7025ceda8f37db3cb9d0ec0fccb07701b82e0aabfc91cc8a249");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // ArtexCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "ArtexCoin Signed Message:\n";
...
{
    // ArtexCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // ArtexCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
	...
	// Mainnet Genesis block:
		...
		const char* pszTimestamp = "ArtexCoin Genesis";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0xc0be56b1e3370828a4a986f06b5ad093159c3f6aca1f04d5c001d5f0e567fcd7"));
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xbf, 0xde }; // ArtexCoin: increase each by adding 6 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ArtexCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ArtexCoinMiner : ProcessBlock, block not accepted");
    }
